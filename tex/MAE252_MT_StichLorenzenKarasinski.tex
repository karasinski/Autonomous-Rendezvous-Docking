...
\begin{quote}
``What is the relative performance between a deliberative and a reactive control architecture in an autonomous rendezvous docking simulation? Additionally, which architecture executes with the highest capture rate while maintaining the lowest fuel use and time to capture?''
\end{quote}
...
Each architecture will be run on many different initial conditions. While the primary importance is the distance from the docking port in the $x$ direction, offsets in the $y$ and $z$ directions will also be investigated. Three outputs from each trial will be measured. The most important output from each trial will be whether or not the target craft is captured, but the total $\Delta v$ required and time to capture will also be compared. Variations in the error introduced to the sensors and thrust will also be investigated. These results will be compared for each architecture to determine which performs the best. 
...
Weeks 6 and 7 will be used to develop the two robotic architectures necessary for implementing the machine vision based 3D pose estimation. Once basic deliberative and reactive architecture algorithms have been completed, integration between the two Python modules will begin. 

Weeks 8 and 9 will be used for final integration of the submodules. At this time several trials will be run to determine the effectiveness of each operational mode, and to determine which mode is the most effective. The effects of various levels of noise in the sensors will be investigated.
